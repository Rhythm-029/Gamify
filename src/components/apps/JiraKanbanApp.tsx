...
export const JiraKanbanApp: React.FC = () => {
    ...
    <div className="flex-1 glass-panel rounded-2xl border border-white/10 flex flex-col overflow-hidden bg-slate-950/80 text-white font-sans text-xs">
      <div className="h-10 bg-slate-900 border-b border-white/10 px-6 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-bold text-white">Jira / Linear — Sprint 1 Kanban Board</span>
        </div>
        <button className="px-3 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white font-bold flex items-center space-x-1 cursor-pointer">
            ...
            <div key={colStatus} className="w-72 sm:w-80 bg-slate-950/60 rounded-2xl border border-white/10 p-4 flex flex-col shrink-0">
              ...
              <div className="flex-1 space-y-3 overflow-y-auto pr-1">
                {colTasks.map((task) => (
                  <div
                    key={task.id}
                    className="p-4 rounded-xl bg-slate-900/90 border border-white/10 hover:border-blue-500/50 transition-all shadow-md cursor-pointer"
                  >
